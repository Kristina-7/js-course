 ...
 * 
 * функция myPrint(a, b, res) – глобальная функция, которая должна генерировать 
 * из параметров a, b, res строку вида 'a^b=res' и возвращать ее
 * 
 * myPrint() должна быть передана в myPow() как параметр 
 * и вызвана внутри как callback-функция
 * 
 ...
 * 
 * пробелы, запятые, символы cc и текст – имеют значение 
 * и проверяются при тестировании кода
 *
 * #6
 *
 ...
 * - если сеттеру used присвоено значение 'new', 
 *   при этом год выпуска автомобиля отличается от текущего года,
...
//let myUniq = (arr) => Array.from(new Set(arr)); novii sintaksis (alternative)
